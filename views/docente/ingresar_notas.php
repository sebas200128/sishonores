...
<body>
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="card">
                        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                            ...
                            <a href="index.php?controller=docente&action=reporte_notas_pdf&curso=<?php echo $curso_seleccionado; ?>&bimestre=<?php echo $bimestre; ?>" target="_blank" class="btn btn-danger btn-sm">
    ...
    <script>
        ...
        function cargarCompetencias() { $.ajax({url: 'index.php?controller=nota', method: 'POST', data: {action:'list_competencias', id_curso:cursoId}, success:function(data){$('#listaCompetencias').html(data);$('#modalCompetencias').modal('show');}}); }
        function agregarCompetencia() { let nombre=$('#nuevaCompetencia').val(); if(nombre.trim()=='') return; $.ajax({url:'index.php?controller=nota',method:'POST',data:{action:'add_competencia',id_curso:cursoId,nombre:nombre},success:function(r){if(r.success){$('#nuevaCompetencia').val('');cargarCompetencias();cargarAlumnos();}}}); }
        function eliminarCompetencia(id) { if(confirm('¿Eliminar?')) $.ajax({url:'index.php?controller=nota',method:'POST',data:{action:'delete_competencia',id_competencia:id},success:function(){cargarCompetencias();cargarAlumnos();}}); }
        function cargarAlumnos() { $.ajax({url:'index.php?controller=nota',method:'POST',data:{action:'alumnos_notas',curso:cursoId,bimestre:bimestre},dataType:'json',success:function(data){
      ...
      function calcularPromedio(input) { 
        ...
        function guardarTodasNotas() { let datos=[]; $('.nota-row').each(function(){ let row=$(this); let templates_notas=[]; row.find('.nota-input').each(function(){templates_notas.push($(this).val()||0);}); datos.push({id_alumno:row.data('id'),notas:templates_notas,observacion:row.find('.observacion').val()}); });
            $.ajax({url:'index.php?controller=nota',method:'POST',data:{action:'guardar_notas',id_docente_curso_aula:cursoId,bimestre:bimestre,datos:JSON.stringify(datos)},success:function(r){if(r.success){alert('Notas guardadas');cargarAlumnos();}else{alert(r.message || 'Error al guardar notas');}}}); }
