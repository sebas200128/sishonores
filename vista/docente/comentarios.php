...
<body>
    ...
    <script>
        function cargarAlumnos() {
            ...
            if(id_curso) {
                $.ajax({url: '../../controller/NotaController.php', method: 'POST', data: {action:'comentarios_alumnos', id_curso:id_curso}, success: function(data){ $('#alumnosList').html(data); }});
            }
        }
        function guardarComentario(id_alumno) {
            let comentario = $('#comentario_'+id_alumno).val();
            let id_curso = $('#selectCurso').val();
            $.ajax({url: '../../controller/NotaController.php', method: 'POST', dataType: 'json', data: {action:'guardar_comentario', id_alumno:id_alumno, id_curso:id_curso, comentario:comentario}, success: function(r){ if(r.success) alert('Comentario guardado'); else alert(r.message || 'No se pudo guardar'); } });
        }
    </script>
</body>
</html>
