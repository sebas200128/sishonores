...
// Verificar permisos de administrador (SuperUsuario, Director, o Secretaria que suele matricular)
...
try {
    ...
    if ($action === 'guardar') {
        ...
        if ($alumnoDAO->checkDniDuplicado($dni, $id)) {
            echo json_encode(['success' => false, 'message' => 'El DNI de este alumno ya se encuentra registrado.']);
            exit;
        }

        // Validar Código duplicado
        if ($alumnoDAO->checkCodigoDuplicado($codigo_estudiante, $id)) {
        ...
        $resultado = $alumnoDAO->guardar($alumno);
